...
%  VERSION BASADA EXCLUSIVAMENTE EN ARCHIVOS .HV CON GRAFICOS
% ================================================================
clc;
close all force;             % Fuerza el cierre real de todas las figuras ocultas
graphics_toolkit('gnuplot'); % Cambia a un motor grafico ligero para evitar caidas
...
for i = 1:length(archivos)
    ...
    try
        ...
        f0 = datos.f0; A0 = datos.A0; Nw = datos.Nw;
        f0_windows = datos.f0_windows; sigma_f = datos.sigma_f;
        frecuencia = datos.frecuencia; HV = datos.HV; HVmin = datos.HVmin; HVmax = datos.HVmax;

        if isnan(f0) || isnan(A0) || isnan(Nw) || isempty(frecuencia)
        ...
        claridad = false(1,6); claridad_estado = cell(1,6);
        [claridad(1), claridad_estado{1}] = criterio_existencia(frecuencia, HV, f0/4, f0, A0/2);
        [claridad(2), claridad_estado{2}] = criterio_existencia(frecuencia, HV, f0, 4*f0, A0/2);
        claridad(3) = A0 > 2;
        if claridad(3), claridad_estado{3} = 'PASS'; else claridad_estado{3} = 'FAIL'; end
        [claridad(4), fpeak_min, fpeak_max] = criterio_fpeak(frecuencia, HVmin, HVmax, f0);
        if claridad(4), claridad_estado{4} = 'PASS'; else claridad_estado{4} = 'FAIL'; end

        epsilon = obtener_epsilon(f0);
        if isnan(sigma_f)
            claridad(5) = false; claridad_estado{5} = 'NO_DATA';
        else
            claridad(5) = sigma_f < epsilon;
            if claridad(5), claridad_estado{5} = 'PASS'; else claridad_estado{5} = 'FAIL'; end
        end

        [sigmaA_f0, theta] = sigmaA_en_f0(frecuencia, HV, HVmin, HVmax, f0);
        if isnan(sigmaA_f0)
            claridad(6) = false; claridad_estado{6} = 'NO_DATA';
        else
            claridad(6) = sigmaA_f0 < theta;
            if claridad(6), claridad_estado{6} = 'PASS'; else claridad_estado{6} = 'FAIL'; end
        end

        cantidad_claridad = sum(claridad);
        ...
        generar_grafico_sesame(frecuencia, HV, HVmin, HVmax, f0, A0, veredicto, cantidad_claridad, nombre_hv, carpeta_graficos);

        fprintf('f0 promedio      : %.5f Hz\nA0                : %.5f\nVEREDICTO: %s\n', f0, A0, veredicto);

        resultados(end+1,:) = {nombre_hv, f0, A0, Nw, LW_SECONDS, nc, C1, C2, C3, cantidad_claridad, ...
            claridad(1), claridad(2), claridad(3), claridad(4), claridad(5), claridad(6), ...
            f0_windows, sigma_f, epsilon, sigmaA_f0, theta, sigmaA_max, fpeak_min, fpeak_max, veredicto};

    catch ME
        fprintf('\nERROR en archivo %s: %s\n', nombre_hv, ME.message);
        resultados(end+1,:) = {nombre_hv, NaN, NaN, NaN, LW_SECONDS, NaN, false, false, false, 0, ...
            false, false, false, false, false, false, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, 'ERROR'};
    end
end

fid = fopen(archivo_salida, 'w');
fprintf(fid, ['Estacion;f0_Hz;A0;Nw;lw_s;nc;C1_frecuencia;C2_ciclos;C3_estabilidad;' ...
     'N_claridad;CL1;CL2;CL3_A0;CL4_fpeak;CL5_sigmaf;CL6_sigmaA;f0_windows;sigma_f;epsilon;sigmaA_f0;theta;sigmaA_max;fpeak_min;fpeak_max;Veredicto\n']);
for i = 1:size(resultados,1)
    fprintf(fid, '%s;%.6f;%.6f;%d;%.3f;%.3f;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%.6f;%.6f;%.6f;%.6f;%.6f;%.6f;%.6f;%.6f;%s\n', resultados{i,:});
end
fclose(fid);
